...
const FurnishedData = [
  { id: 1, name: "Fully Furnished", value: "Fully Furnished" },
  { id: 1, name: "half Furnished", value: "half Furnished" },
  { id: 1, name: "semi Furnished", value: "semi Furnished" },
]
...
const Furnished = () => {
  ...
  const [editingFurnished, setEditingFurnished] = useState(null);
  ...
  const fetchFurnished = async () => {
    ...
    //   message.success('Country codes fetched successfully!');
      ...
      console.error('Error fetching Furnished:', error);
      message.error('Error fetching Furnished.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleStatusToggle = async(record)=>{
    try {
       const response = await axios.patch(`${baseurl}/api/furnished/toggled/${record._id}`)
  ...
  const handlePut = async(values)=>{
    ...
    try {
      const response = await axios.put(`${baseurl}/api/furnished/updateFurnished/${editingFurnished._id}`,postdata);
      console.log(response.data);
  ...
  return (
    <div>
      ...
      />
       ...
       <Modal
        title={editingFurnished ? 'Edit Furnished' : 'Add Furnished'}
        open={isModalOpen}
        ...
        >
          ...
          {FurnishedData.map(loc => (
            ...
            <Button type="primary" htmlType="submit">
              {editingFurnished ? 'Update' : 'Submit'}
            </Button>
          </Form.Item>
