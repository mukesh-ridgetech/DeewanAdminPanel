...
const BlogList = () => {
  ...
  useEffect(() => {
    const fetchBlogsAuthorsTags = async () => {
      try {
        const [blogsResponse, authorsResponse, tagsResponse] =
          await Promise.all([
  ...
  const showEditModal = (blog) => {
    ...
    form.setFieldsValue({
      ...
      tags: blog.tags?.map((tag) => tag), // Update to set tag IDs
      slug: blog.slug,
  ...
  const handleUpdate = async () => {
    try {
      const updatedBlog = {
        ...form.getFieldsValue(),
        content,
        image: imageSourceType === "url" ? imageUrl : fileList[0]?.url,
        slug,
        modifiedBy: auth.user._id,
      };
      await axios.put(`${baseurl}/api/blog/${currentBlog._id}`, updatedBlog);
